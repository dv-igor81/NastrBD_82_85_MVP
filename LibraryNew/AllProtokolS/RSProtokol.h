...
// Поддерживается ли выбранная функция прошивкой?
// 1. Пока что не известно
// 2. Не поддерживается
// 3. Поддерживается
enum SupportType_Tag { UnknowSupport, NotSupport, IsSupport };
typedef SupportType_Tag SupportType;
//---------------------------------------------------------------------------
class Data_t
{
public:
  ...
  // ===>> ===>> ===>> 15.10.2019 ===>> ===>> ===>>
  SupportType DoseSupport;
  unsigned int Dose; // Доза
  // <<=== <<=== <<=== 15.10.2019 <<=== <<=== <<===
};
...
// или это --> (U0 * 0,81 + Un == Uhi)
  ...
  TaskExecutionStatus SpectrStatus;
public:
  ...
  int GetDoseMul_100(unsigned int * Dose);
  ...
  void OprosSpectraStatus(
    TaskExecutionStatus teStatus);

  unsigned char _kolTikSpectr;
