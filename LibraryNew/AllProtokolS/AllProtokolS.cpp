...
//---------------------------------------------------------------------------
 // Инициализация МК в БД (интервал набота счета, задать колтик = 8)
bool AllProtokolS::InitMkInBd()
{
    unsigned char timeInterval = 0;
    const unsigned char defaultTimeInterval = 8;
    if ( GetTimeInterval( & timeInterval ) == false )
    {
        return false;
    }
    if ( timeInterval != defaultTimeInterval )
    {
        if ( SetTimeInterval( defaultTimeInterval ) == false )
        {
            return false;
        }
        if ( GetTimeInterval( & timeInterval ) == false )
