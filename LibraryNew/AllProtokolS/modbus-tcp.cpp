...
getaddrinfo_t * getaddrinfo = NULL;
freeaddrinfo_t * freeaddrinfo = NULL;
...
static int _modbus_tcp_set_ipv4_options(int s)
{
    ...
    rc = setsockopt(s, IPPROTO_TCP, TCP_NODELAY,
                    (const char *)(const void *)&option, sizeof(int));
    if (rc == -1) {
        return -1;
    }
    ...
    for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
        ...
        if (s < 0) {
            ...
            rc = setsockopt(s, SOL_SOCKET, SO_REUSEADDR,
                            (const char *)(void *) &yes, sizeof (yes));
            if (rc != 0) {
                close(s);
                if (ctx->debug) {
                    perror("setsockopt");
...
int modbus_tcp_pi_accept(modbus_t *ctx, int *socket)
{
    ...
    addrlen = sizeof(addr);
    ctx->s = accept(*socket, (sockaddr *)(void *)&addr, &addrlen);
    if (ctx->s == -1) {
        close(*socket);
        *socket = 0;
    }
...
// ===>> 17.07.2017
const modbus_backend_t _modbus_rtu_tcp_ip_backend = {
    _MODBUS_BACKEND_TYPE_TCP,
    _MODBUS_RTU_TCP_IP_HEADER_LENGTH,
    _MODBUS_RTU_TCP_IP_CHECKSUM_LENGTH,
    MODBUS_TCP_MAX_ADU_LENGTH,
    _modbus_set_slave,
    _modbus_tcp_build_request_basis,
    _modbus_tcp_build_response_basis,
...
// ===>> 17.07.2017
modbus_t * modbus_new_rtu_tcp_ip(const char * ip, int port)
{
  modbus_t * ctx;
  ...
  ctx->backend = &(_modbus_rtu_tcp_ip_backend); //! ===!!!=== ===!!!===
