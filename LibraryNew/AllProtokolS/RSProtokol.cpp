...
void RSProtokol_t::Reset(void)
{
  ...
  this->Data.DoseSupport = UnknowSupport; // 15.10.2019
...
//---------------------------------------------------------------------------
// Считать счёт за определённый (програмно установленный) временной интервал,
// например за 0,2 сек
...
//---------------------------------------------------------------------------
// Считать ДОЗУ, разделив ее на 100 получим микрозиверты
int RSProtokol_t::GetDoseMul_100(unsigned int * Dose)
{
  unsigned int data;
  this->buf_write[0] = 0x26;
  this->CodeRet = CommandExec(0, 2);
  if (this->CodeRet == 0) // Нет ошибки обмена
  {
    data = this->buf_read[1];
    *Dose = data * 256 + this->buf_read[0];
  }
  else // Ошибка обмена
  {
    *Dose = 0;
    ...
    if ( ( Data.PSW & 0x01 ) == 0x01 ) // Флаг готовности счёта получен
    {
      ...
      if ( GetCountImp(&Data.CountImp) == 0 )
      {
        ...
        if ( Data.flagFirst == 0 )
        {
          ...
          {
          case 0: // 0 - "9-ти битный"
            if ( Data.DoseSupport != NotSupport )
            {
              if ( GetDoseMul_100(&Data.Dose) == 0 )
              {
                if ( Data.DoseSupport == UnknowSupport )
                {
                  Data.DoseSupport = IsSupport;
                }
              }
              else
              { // Функция получения дозы не поддерживается
                Data.DoseSupport = NotSupport;
              }
            }
            break;
          case 1: // 1 - ModBus RTU
...
int RSProtokol_t::OprosBD( void ) // -1 - ошибка связи, 0 - звязь работает
{
  ...
  // ===>> ===>> ===>> 14.10.2019 ===>> ===>> ===>>
  // Теперь спектр собирается по другому, поэтомк
  // фрагмент кода ниже будет закоментирован
  if ( bfOprosSpectra == true )
  {
    //  iRetValue = OprosBDSpektr(
    //    true,
    //    & Data.TimeNaboraSpectr,
    //    & Data.TimeGotovSpectr,
    //    _kolTikSpectr );



    iRetValue = OprosBDSpektr_New( true );
    if ( iRetValue != 0 )
    {
      bFlagWork = false;
      return iRetValue;
  ...
  else if ( SpectrStatus == PreparingForTaskCompletion )
  {
    SpectrStatus = TaskNotCompleted;
    iRetValue = OprosBDSpektr_New( false );
  }
  // <<=== <<=== <<=== 14.10.2019 <<=== <<=== <===
...
//---------------------------------------------------------------------------
void RSProtokol_t::OprosSpectra(
  bool bf, // bf = true - разрешить опрос спектра в функции OprosBD()
  unsigned char kolTikSpectr)
{
  bfOprosSpectra = bf;
  _kolTikSpectr = kolTikSpectr;
  if ( bf == true )
  {
...
//---------------------------------------------------------------------------
void RSProtokol_t::OprosSpectraStatus(
  TaskExecutionStatus teStatus)
{
  SpectrStatus = teStatus;
}
//---------------------------------------------------------------------------
...
int RSProtokol_t::OprosBDSpektr_New(
    bool bfStartStop) // true - Старт набота спектра, false - Стоп набора
                      // спектра, и сохранение спектра в массив
{
  int iRet = 0;
  int i;
  switch( flagModbusProtokol )
  ...
  case 0: // 0 - "9-ти битный"
    ...
    if( bfStartStop == false )
    {
      ...
      for (int nomBlock = 0; nomBlock < 32 * 2; nomBlock++) //==\\
      {
        ...
        for (i = 0; i < 16; i += 2)
        {
          this->Data.ArrSpectr[nomBlock * 8 + i/2] = 0;
...
//---------------------------------------------------------------------------
// Возврат: -1 - ошибка связи, 0 - звязь работает нормально
int RSProtokol_t::OprosBDSpektr(
    bool bfTime,
    int * iTimeRead,
    int * iTimeGotov,
    unsigned char kolTikSpectr)
{
  ...
  {
    Data.KolTikSpectr = kolTikSpectr;

    // ===>> ===>> ===>> ===>> 12.10.2019 ===>> ===>> ===>> ===>>
    // Из БД удалена возможность изменять интервал набора спектра
    // поэтому код ниже будет закомментирован
