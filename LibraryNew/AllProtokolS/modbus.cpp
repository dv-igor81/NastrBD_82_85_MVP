  ...
  else
  {
    ...
    switch ( function )
    {
      ...
      case _FC_WRITE_MULTIPLE_REGISTERS:
        ...
        break;
// DIA ===>>
      case _FC_KOD_FUNK_USERS_65DEC_41HEX:
        length = ModBus_kInByte;
        break;
// DIA <<===
      default:
        length = 1;
...
static int compute_data_length_after_meta(
    modbus_t * ctx,
    uint8_t * msg,
    msg_type_t msg_type)
{
  ...
  else
  {
    ...
    // DIA ===>>
    else if (  function == _FC_KOD_FUNK_USERS_65DEC_41HEX )
    {
      length = 0;    
    }
    // DIA <<===
      ...
      switch (step)
      {
        case _STEP_FUNCTION:
          ...
          length_to_read = compute_meta_length_after_function(
              msg[ctx->backend->header_length], // Êîä ôóíêöèè
              msg_type);
          if (length_to_read != 0)
          {
            step = _STEP_META;
...
{
    int rc;
    unsigned int rsp_length_computed;
    const int offset = ctx->backend->header_length;
                          ...
                          uint16_t *dest)
{
    int rc;
    int req_length;
    uint8_t req[_MIN_REQ_LENGTH]; // #define _MIN_REQ_LENGTH 12
    uint8_t rsp[MAX_MESSAGE_LENGTH]; // #define MAX_MESSAGE_LENGTH 260
